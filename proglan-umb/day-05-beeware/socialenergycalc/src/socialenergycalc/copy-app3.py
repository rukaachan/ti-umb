...
from toga import App, MainWindow, Box, Label, Selection, TextInput, Button, Table, Divider, ConfirmDialog, InfoDialog, ImageView, Image
...
COLOR_ACTION_BUTTON_BG = '#FFA500' # Hex code for Orange
...
class SocialEnergyCalculator(toga.App):
    ...
    def startup(self):
        ...
        input_section_title = Label("📝 Log New Activity", style=Pack(font_weight=BOLD, padding_bottom=PADDING_MEDIUM, color=COLOR_TEXT_PRIMARY))
        ...
        # FIX: Simplified heading
        self.activity_table = Table(
            headings=["Activity Type", "Duration (hr)", "Points"], # Changed heading
            missing_value="-",
            style=Pack(flex=1)
        )
        ...
        balance_title = Label("⚡️ Current Energy Balance", style=Pack(font_weight=BOLD, padding_bottom=PADDING_SMALL, color=COLOR_TEXT_PRIMARY, text_align=CENTER))
        ...
        mood_image_title = Label("😊 Mood:", style=Pack(font_weight=BOLD, padding_bottom=PADDING_SMALL, color=COLOR_TEXT_PRIMARY, text_align=CENTER))
        self.mood_image_view = ImageView(style=Pack(width=100, height=100, alignment=CENTER))
    ...
    def add_activity(self, widget):
        ...
        try:
            ...
            # Optional positive duration check (remove if not needed)
            # if duration <= 0:
    ...
    def update_display(self):
        """Updates table (today's entries), energy balance label, and mood image."""
        today_str = date.today().isoformat()
        todays_table_data = []

        for activity in self.all_activities:
            if activity.get("date") == today_str:
                # FIX: Changed dictionary key to "points" to match simplified heading
                todays_table_data.append({
                    "activity_type": activity.get("type", "N/A"),
                    "duration_hr": f"{float(activity.get('duration', 0.0)):.2f}",
                    # Use the simplified key "points" but keep the correct formatting
        ...
        if 71 <= current_balance <= 100: target_image_name = SMELL_IMAGE_NAME
        elif 31 <= current_balance <= 70: target_image_name = COFFEE_IMAGE_NAME
        elif 0 <= current_balance <= 30: target_image_name = REST_IMAGE_NAME
        ...
        try:
            ...
            elif default_path.exists(): loaded_image = toga.Image(default_path); print(f"Warning: Image {target_image_name} not found. Using default.")
            else: print(f"Warning: Target ({target_image_name}) and default images not found. Clearing image.")

            if hasattr(self, 'mood_image_view'): self.mood_image_view.image = loaded_image
            else: print("Error: self.mood_image_view widget not found during update.")
            ...
            try:
                with data_file_path.open("r", encoding='utf-8') as f:
                    ...
                    if isinstance(data, dict) and 'current_balance' in data and 'activity_log' in data:
                        ...
                        if isinstance(data['activity_log'], list): self.all_activities = data['activity_log']
                        else: print("Warning: Loaded 'activity_log' is not a list. Ignoring."); self.all_activities = []
